...
class Instance {
            ...
            for (var url of args) {
                ...
                if (main_ytdl.validateURL(url)) {
                    ...
                    if (!url.includes("list")) {
                        ...
                        if (autoplay) this.Play(this.last_channel);
                    }
    ...
    static RemoveInstance(guild_id) {
        for (var i = 0; i < instances.length; i++)
            if (instances[i].guild_id = guild_id)
                instances.splice(i, 1);
    }

    static TryFetchInstance(guild_id) {
        for (let i = 0; i < instances.length; i++)
            if (instances[i].guild.id === guild_id)
                return instances[i];
    ...
    switch (args[0]) {
        case "play":
            ...
            if (!instance) {
                ...
                instances[instances.length - 1].AddToQueue(args_with_case, true);
            } else {
        ...
        case "mix":
            ...
            if (!instance) {
                instances.push(new Instance(message));
                instances[instances.length - 1].AddToQueue(urls);
                instances[instances.length - 1].Play(message.channel);
            } else {
                instance.AddToQueue(urls);
                instance.Play(message.channel);
            }
