...
class Instance {
    ...
    NextTrack(count: number = 1): void {
        if (count < 1) return;
        if (count >= this.tracklist.length) return;
    ...
    async SendControlPanel(): Promise<void> {
        ...
        this.interactioncollector.on("collect", i => {
            switch (i.customId) {
                case "play":
                    this.UnPause();
                    break;
    ...
    PlayerIdlingHandler(): void {
        if (this.tracklist.length < 1) console.log(`${Utils.GetTimeStamp()} Finished streaming to ${this.voicechannel.guild.name}->${this.voicechannel.name}`);
        else {
    ...
    ControlsHandler(i: Discord.MessageComponentInteraction): void {
        try {
            switch (i.customId) {
                case "play":
                    this.UnPause();
