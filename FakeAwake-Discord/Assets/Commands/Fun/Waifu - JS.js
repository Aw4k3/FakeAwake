...
async function AddWaifuToEmbed(waifu_type) {
    image = null;
...
function Run(message, args, args_with_case, client) {
    if (message.content.includes("flag")) {
        ...
        for (let i = 1; i < args.length; i++) {
            g_waifu_type = args[i];
            AddWaifuToEmbed(args[i]).then(embed => {
                var g_embed = embed;
                ...
                message.channel.send(context).then(() => {
                    ...
                    client.on("interactionCreate", interaction => {
                        if (interaction.customId === "waifu-type") {
                            ...
                            AddWaifuToEmbed(interaction.values[0]).then(embed => {
                        ...
                        if (interaction.customId === "next") {
                            AddWaifuToEmbed(waifu_type).then(new_embed => {
                                g_embed = new_embed;
